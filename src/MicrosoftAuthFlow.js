...
class MicrosoftAuthFlow {
  constructor (username = '', cache = __dirname, options, codeCallback, agent = undefined) {
  ...
  initTokenManagers (username, cache) {
    ...
    this.xbl = new XboxTokenManager(keyPair, cache({ cacheName: 'xbl', username }), this.agent)
    this.mba = new BedrockTokenManager(cache({ cacheName: 'bed', username }))
    this.mca = new JavaTokenManager(cache({ cacheName: 'mca', username }), this.agent)
  }
