...
class MinecraftJavaTokenManager {
  constructor (cache, agent = undefined) {
    this.cache = cache
    this.agent = agent;
  }
  ...
  async fetchProfile (accessToken) {
    ...
    const profile = await fetch(Endpoints.MinecraftServicesProfile, { headers, agent: this.agent })
      .then(checkStatus)
 ...
 */
  async fetchEntitlements (accessToken) {
    ...
    const headers = { ...fetchOptions.headers, Authorization: `Bearer ${accessToken}` }
    const entitlements = await fetch(Endpoints.MinecraftServicesEntitlement, { headers, agent: this.agent }).then(checkStatus)
  ...
  async fetchCertificates (accessToken) {
    ...
    const headers = { ...fetchOptions.headers, Authorization: `Bearer ${accessToken}` }
    const cert = await fetch(Endpoints.MinecraftServicesCertificate, { method: 'post', headers, agent: this.agent }).then(checkStatus)
